...
pub async fn clone_agent(
	...
	// Get original agent
	let original = AgentBmc::get_cached(&ctx.0, &mm, id).await?;

	// Create cloned agent with new name
	let agent_c = AgentForCreate {
		name: payload.new_name,
	};
	let cloned_id = AgentBmc::create_cached(&ctx.0, &mm, agent_c).await?;
	let cloned_agent = AgentBmc::get_cached(&ctx.0, &mm, cloned_id).await?;
...
pub async fn get_agent_stats(
	...
	// Get total count
	let total_count = AgentBmc::count_cached(&ctx.0, &mm, None).await?;
...
pub async fn export_agent(
	...
	let ctx = ctx.0;
	let agent = AgentBmc::get_cached(&ctx, &mm, id).await?;
...
pub async fn archive_agent(
	...
	let _agent = AgentBmc::get_cached(&ctx.0, &mm, id).await?;

	Ok(ArchiveAgentResponse { id, archived: true }.into())
}
...
pub async fn agent_report(
	...
	let total = AgentBmc::count_cached(&ctx.0, &mm, None).await?;
...
pub async fn view_agent(
	...
	// Permission check is automatically injected!
	let agent = AgentBmc::get_cached(&ctx.0, &mm, id).await?;
	Ok(agent.into())
}
